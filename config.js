let path = location.pathname.split("/").filter(a => a.length > 0)[0];
feed_server_path = `https://api.codemap.ai/spotboard/${path}/`;
...
config =
	{
		...
		award_animation_speeds: [
			{
				solved: 10, fastFlipSpeed: 1200, slowFlipSpeed: 2000, utuSpeedFunc: function (up_cnt) {
					return Math.min(500 + up_cnt * 100, 2000);
				}
			},
			{
				solved: 9, fastFlipSpeed: 800, slowFlipSpeed: 1200, utuSpeedFunc: function (up_cnt) {
					return Math.min(500 + up_cnt * 100, 2000);
				}
			},
			{
				solved: 7, fastFlipSpeed: 400, slowFlipSpeed: 600, utuSpeedFunc: function (up_cnt) {
					return Math.min(500 + up_cnt * 100, 2000);
				}
			},
			{
				solved: 3, fastFlipSpeed: 200, slowFlipSpeed: 300, utuSpeedFunc: function (up_cnt) {
					return Math.min(250 + up_cnt * 50, 3000);
				}
			},
			{
				solved: 0, fastFlipSpeed: 200, slowFlipSpeed: 300, utuSpeedFunc: function (up_cnt) {
